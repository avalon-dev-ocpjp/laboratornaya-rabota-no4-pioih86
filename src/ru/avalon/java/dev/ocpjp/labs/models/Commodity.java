...
public interface Commodity {
    ...
    static Collection<Commodity> random(int limit) throws IOException {
        try (RandomFileReader reader = RandomFileReader.fromSystemResource("resources/household.csv")) {
             ...
             */
            return Stream.generate(reader::readLine).limit(limit).
                    filter(x -> x.split(";").length == 5).map(x -> valueOf(x)).collect(Collectors.toList());
        }
    }
    ...
    static Commodity valueOf(String string) {
        ...
        return Commodity.builder().code(parsedString[0]).vendorCode(parsedString[1]).name(parsedString[2]).
                residue(Integer.valueOf(parsedString[3])).price(Double.valueOf(parsedString[4])).build();
    }
